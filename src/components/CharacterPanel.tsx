  ...
  return (
    <div className="flex h-full w-80 shrink-0 flex-col border-l border-gray-200 bg-white">
      ...
      <div className="flex items-center justify-between border-b border-gray-200 px-4 py-3">
        <span className="font-bold text-gray-900">我的书游档案</span>
        <div className="flex items-center gap-1">
          <div className="relative">
            ...
            {menuOpen && (
              <div className="absolute right-0 z-20 mt-1 w-48 rounded border border-gray-200 bg-white py-1 text-xs shadow-lg">
                <button
                  onClick={() => {
                    exportArchive();
                    setMenuOpen(false);
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600" title="关闭">
            ✕
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 text-sm leading-relaxed">
        {tab === "profile" && (
          <div className="space-y-3">
            ...
            <div>
              <div className="mb-1 text-gray-400">开局设定</div>
        ...
        {tab === "power" && (
          <div className="space-y-3">
            ...
            <div>
              <div className="mb-1 text-gray-400">掌控资源</div>
        ...
        {tab === "history" && (
          <div className="space-y-4">
            <div>
              <div className="mb-2 font-medium text-gray-700">大事（颠覆性行动）</div>
              {bigEvents.length ? (
                <ul className="space-y-2">
                  {bigEvents.map((a) => (
                    <li key={a.id} className="rounded border-l-2 border-red-400 bg-red-50 px-2 py-1">
                      <span className="text-gray-700">「{a.content}」</span>
                      <span className="text-xs text-gray-400"> · 第{a.chapterIndex + 1}章</span>
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="text-gray-400">暂无。做一件复杂行动就会被记在这里。</p>
              )}
            </div>

            <div>
              <div className="mb-2 font-medium text-gray-700">流水账</div>
              {recentActions.length ? (
                ...
                <p className="text-gray-400">暂无行动。</p>
              )}
            </div>
          </div>
        )}
              ...
              npcs.map((m) => (
                <div key={m.id} className="rounded border border-gray-200 p-3">
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-gray-900">{m.npcName}</span>
                    <span className="text-xs text-gray-500">{m.attitude}</span>
                  </div>
        ...
        {tab === "offset" && (
          <div className="space-y-4">
            <div className="text-center">
              <div className="text-4xl font-bold text-gray-900">{offset.toFixed(2)}</div>
              <div className="mt-1 text-gray-600">{offsetTier(offset)}</div>
            </div>
